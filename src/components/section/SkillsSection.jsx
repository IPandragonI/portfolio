                ...
                <div className='w-full h-1/6 md:w-1/4 md:h-full flex flex-row md:flex-col justify-evenly'>
                    {articles.map((section, index) => (
                        <div
                            ...
                            className={`text-sm md:text-xl lg:text-4xl w-1/4 md:w-auto h-16 md:h-[unset] py-4 px-2 text-center font-bold md:py-8 md:px-2 ${index === active ? 'bg-secondary' : 'bg-primary hover:bg-primary-hover'} text-white rounded-3xl cursor-hover transition ease-in delay-50`}
                        >
                            {section.name}
                        </div>
                    ))}
